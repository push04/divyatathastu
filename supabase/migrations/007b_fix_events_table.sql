-- =============================================
-- MIGRATION 007b — Fix events table schema
-- =============================================
-- If an events table already existed without is_published / is_free columns,
-- 007 skipped CREATE TABLE and then the policy creation failed.
-- This migration drops and cleanly recreates the events table.
-- =============================================

-- Drop existing (wrong-schema) table and its policies
...
-- Ensure handwritten_report_requests exists (in case 007 partially failed)
...
DROP POLICY IF EXISTS "hrr_user_read_own" ON public.handwritten_report_requests;
DROP POLICY IF EXISTS "hrr_user_insert" ON public.handwritten_report_requests;
DROP POLICY IF EXISTS "hrr_admin_all" ON public.handwritten_report_requests;
...
DROP POLICY IF EXISTS "hr_reports_user_read" ON storage.objects;
DROP POLICY IF EXISTS "hr_reports_admin_all" ON storage.objects;
