 ...
 * interprets this as an exit+enter, causing the flash/vanish effect.
 *
 * SOLUTION: Always keep the modal in the DOM. Drive visibility purely via
 * framer-motion's `animate` prop (opacity + pointer-events + transform).
 * The lastTempleRef holds the last valid temple data so content stays rendered
 * during the fade-out transition even after `temple` prop is set to null.
 */
export default function TempleDetailModal({ temple, isOpen, onClose }: TempleDetailModalProps) {
  // Cache the last valid temple so content is still renderable during close animation
  const lastTempleRef = useRef<Temple | null>(null)
  useEffect(() => {
    if (temple) lastTempleRef.current = temple
  }, [temple])
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    {displayTemple.special_events.map(event => (
                      <span key={event} className="text-xs px-3 py-1 rounded-lg bg-[var(--warm-sand)]/30 text-[var(--indigo-deep)] border border-[var(--warm-sand)] font-medium">
                        {event}
                      </span>
                    ))}
                  </div>
