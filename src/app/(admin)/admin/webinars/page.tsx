    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)] mt-0.5">Create group live sessions · Generate join links · Send invites</p>
        </div>
        <button onClick={() => setShowCreate(s => !s)} className={btnPrimary}>
          <Icon name="add" size={16} className="align-middle mr-1" />
          New Webinar
        </button>
      </div>
      ...
      {showCreate && (
        <div className="card-divine p-5 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs text-[var(--text-secondary)] mb-1 block">Host Name</label>
              <input className={inputCls} value={form.host_name} onChange={e => setForm(f => ({ ...f, host_name: e.target.value }))} />
            </div>
            <div>
              <label className="text-xs text-[var(--text-secondary)] mb-1 block">Schedule Date & Time</label>
            ...
            <div>
              <label className="text-xs text-[var(--text-secondary)] mb-1 block">Max Participants</label>
            ...
            <div>
              <label className="text-xs text-[var(--text-secondary)] mb-1 block">Price (₹, 0 = free)</label>
      ...
      {/* Webinar list */}
      {loading ? (
        <div className="text-center py-12 text-[var(--text-muted)] text-sm">Loading webinars...</div>
      ) : webinars.length === 0 ? (
        <div className="card-divine p-10 text-center">
          <Icon name="live_tv" size={37} className="text-[var(--warm-sand)] block mb-3" />
          <p className="text-[var(--text-muted)]">No webinars yet. Create one to get started.</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span className={`text-[12px] font-semibold px-2 py-0.5 rounded-full border capitalize ${STATUS_COLORS[w.status]}`}>
                      {w.status === 'live' && <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1 animate-pulse" />}
                      {w.status}
                    </span>
                    {w.price > 0 && <span className="text-[12px] bg-amber-50 text-amber-700 border border-amber-200 px-2 py-0.5 rounded-full">₹{w.price}</span>}
                    {w.price === 0 && <span className="text-[12px] bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-full">Free</span>}
                  </div>
                  ...
                  {w.description && <p className="text-xs text-[var(--text-secondary)] line-clamp-2">{w.description}</p>}

                  {/* Join link */}
                  <div className="mt-3 flex items-center gap-2 bg-[var(--warm-sand)]/40 rounded-xl px-3 py-2">
                    <Icon name="link" size={14} className="text-[var(--saffron)]" />
                    <a
                      href={`/webinar/${w.id}`}
                      target="_blank"
                    ...
                    >
                      <Icon name={copiedId === w.id ? 'check' : 'content_copy'} size={14} />
                ...
                <div className="flex flex-col gap-2 items-end shrink-0">
                  {/* Status toggle.
                      "Open Host Studio" goes to the same /webinar/[id] page,
                      which now detects the host and shows Go Live + Enter
                      Studio controls instead of the attendee card. */}
                  <div className="flex gap-1">
                    {w.status !== 'live' && (
                      <>
                        <button onClick={() => updateStatus(w.id, 'live')} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-500 text-white text-xs font-semibold hover:bg-emerald-600 transition-colors">
                          <Icon name="videocam" size={14} /> Go Live
                        </button>
                        <a
                          href={`/webinar/${w.id}`}
                          target="_blank"
                        ...
                        >
                          <Icon name="stars" size={14} /> Host Studio
                        </a>
                      </>
                    )}
                    {w.status === 'live' && (
                      <>
                        <a
                          ...
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-indigo-600 text-white text-xs font-semibold hover:bg-indigo-700 transition-colors"
                        >
                          <Icon name="meeting_room" size={14} /> Enter Studio
                        </a>
                    ...
                    {w.status === 'ended' && (
                      <button onClick={() => updateStatus(w.id, 'upcoming')} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-amber-500 text-white text-xs font-semibold hover:bg-amber-600 transition-colors">
                        <Icon name="restart_alt" size={14} /> Reschedule
                      </button>
                    )}
                  </div>
      ...
      {inviteWebinar && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <p className="text-xs text-[var(--text-muted)] mb-4">{inviteWebinar.title}</p>

            <div className="mb-3">
              <label className="text-xs text-[var(--text-secondary)] mb-1 block">Recipient Name (optional)</label>
              <input className={inputCls} placeholder="Seeker" value={inviteName} onChange={e => setInviteName(e.target.value)} />
            </div>
            <div className="mb-4">
              <label className="text-xs text-[var(--text-secondary)] mb-1 block">Email Addresses</label>
              <textarea
                className={inputCls}
                rows={4}
              ...
              />
              <p className="text-[12px] text-[var(--text-muted)] mt-1">Enter multiple addresses - one per line or comma-separated</p>
            </div>
