    ...
    <div className="min-h-screen bg-[var(--kutch-white)]">
      ...
      <div style={{ background: 'linear-gradient(135deg, #fdf6e3 0%, #fef9ed 60%, #faf3de 100%)', borderBottom: '2px solid rgba(212,160,23,0.3)' }}>
        <div className="max-w-5xl mx-auto px-6 py-16 text-center">
          <div className="w-24 h-24 rounded-full bg-[#6b3a2a] flex items-center justify-center text-white text-5xl font-black mx-auto mb-6 shadow-xl" style={{ fontFamily: "var(--font-display)" }}>G</div>
          <h1 className="text-5xl font-black text-[#2c1a0e] mb-2 tracking-wide" style={{ fontFamily: "var(--font-display)" }}>GYANAMPEETHAM</h1>
          ...
          <div className="mt-4 inline-flex items-center gap-4 text-xs text-[var(--text-muted)] uppercase tracking-widest">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 space-y-12">
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-[var(--indigo-deep)]" style={{ fontFamily: "var(--font-display)" }}>Programs & Satsangs</h2>
          ...
          {loading ? (
            ...
            <div className="bento-card p-10 text-center">
              <p className="text-[var(--text-muted)]">Programs coming soon. Check back shortly.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {courses.map(item => {
                ...
                return (
                  <div key={item.id} className="bento-card p-5 flex flex-col hover:shadow-lg transition-all hover:-translate-y-0.5">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <span className="text-[12px] px-2 py-0.5 rounded-full font-semibold" style={{ background: lc.bg, color: lc.text }}>{item.level || 'Open'}</span>
                    ...
                    {item.duration && <p className="text-[13px] text-[var(--text-muted)] mb-1" style={{ fontFamily: "var(--font-label)" }}>{item.duration}</p>}
                    <h3 className="font-bold text-[var(--indigo-deep)] mb-2 text-sm flex-1" style={{ fontFamily: "var(--font-display)" }}>{item.title}</h3>
                    {item.description && <p className="text-xs text-[var(--text-secondary)] leading-relaxed mb-3 flex-1">{item.description}</p>}
                    {item.instructor_name && <p className="text-[13px] text-[var(--saffron)] mb-3">👤 {item.instructor_name}</p>}
        ...
        <section style={{ background: 'linear-gradient(135deg, #fdf6e3, #fef9ed)', borderRadius: 20, padding: '40px', border: '1px solid rgba(212,160,23,0.3)' }}>
          <div className="text-center mb-6">
            ...
            <h2 className="text-xl font-bold text-[#2c1a0e]" style={{ fontFamily: "var(--font-display)" }}>ज्योतिष शास्त्र के 18 आचार्य</h2>
        ...
        <div className="text-center space-y-3">
          <p className="text-sm text-[var(--text-secondary)]">For admissions, partnerships, and queries</p>
          <div className="flex gap-3 justify-center flex-wrap">
            ...
            <Link href="/consultations" className="btn-outline-divine inline-flex items-center gap-2 px-6 py-3">
              <Icon name="event" size={18} /> Book 1-on-1 Guidance
            </Link>
          </div>
        </div>
      </div>
