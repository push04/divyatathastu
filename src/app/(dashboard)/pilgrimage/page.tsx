...
export default function PilgrimagePage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[var(--indigo-deep)] inline-flex items-center gap-2"><Icon name="train" size={24} /> Pilgrimage Itinerary</h1>
        <p className="text-sm text-[var(--text-secondary)] mt-1">Sacred journey planner</p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div className="card-divine p-5 space-y-4">
              <h2 className="text-lg font-bold text-[var(--indigo-deep)]" style={{ fontFamily: "var(--font-display)" }}>Journey Details</h2>
              <div>
              ...
              <div>
                ...
                <div className="flex justify-between text-xs text-[var(--text-muted)] mt-1"><span>3</span><span>21 days</span></div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {TRAVEL_MODES.map(m => (
                    <button key={m} onClick={() => setTravelMode(m.toLowerCase())}
                      className={`py-1.5 rounded-lg text-xs font-medium transition-all ${travelMode === m.toLowerCase() ? 'bg-[var(--indigo-deep)] text-white' : 'bg-white border border-[var(--warm-sand)] text-[var(--text-secondary)] hover:border-[var(--indigo-deep)]'}`}>
                      {m}
                    </button>
                  ))}
                </div>
              ...
              <button onClick={generate} disabled={generating} className="btn-divine w-full py-3 font-bold disabled:opacity-50">
                ...
                ) : <span className="inline-flex items-center gap-2"><Icon name="brightness_7" size={18} /> Plan Pilgrimage ({selected.size} temples)</span>}
              </button>
            </div>

            {savedItineraries.length > 0 && (
              <div className="card-divine p-5">
                <h2 className="text-lg font-bold text-[var(--indigo-deep)] mb-3" style={{ fontFamily: "var(--font-display)" }}>Previous Itineraries</h2>
                <div className="space-y-2">
                  {savedItineraries.map(s => (
                    <button key={s.id} onClick={() => setItinerary(s.schedule as any)} className="w-full text-left p-2.5 rounded-lg hover:bg-[var(--warm-sand)] transition-all">
                      ...
                      <p className="text-xs text-[var(--text-muted)]">{new Date(s.created_at).toLocaleDateString('en-IN')}</p>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-bold text-[var(--indigo-deep)]" style={{ fontFamily: "var(--font-display)" }}>Select Temples ({selected.size} chosen)</h2>
              <button onClick={() => setSelected(new Set())} className="text-xs text-[var(--text-muted)] hover:text-[var(--terracotta)]">Clear all</button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[600px] overflow-y-auto pr-1">
              {mandirs.map(m => (
                ...
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm flex-shrink-0 ${selected.has(m.id) ? 'bg-[var(--terracotta)] text-white' : 'bg-[var(--warm-sand)] text-[var(--text-muted)]'}`}>
                    <Icon name={selected.has(m.id) ? 'check' : 'temple_hindu'} size={16} />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[var(--text-muted)] mt-0.5">{m.deity}</p>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-[var(--text-secondary)]">{itinerary.totalDays} days · Starting from {itinerary.startCity}</p>
            </div>
            <button onClick={() => setItinerary(null)} className="text-sm text-[var(--terracotta)] hover:underline inline-flex items-center gap-1"><Icon name="arrow_back" size={16} />New Plan</button>
          </div>

          {/* Highlights */}
          {itinerary.highlights?.length > 0 && (
            <div className="card-divine p-5">
              <h3 className="font-bold text-[var(--indigo-deep)] mb-3 inline-flex items-center gap-1"><Icon name="brightness_7" size={18} /> Highlights</h3>
              <div className="flex gap-2 flex-wrap">
                {itinerary.highlights.map(h => <span key={h} className="text-sm bg-[var(--warm-sand)] px-3 py-1 rounded-full text-[var(--text-secondary)]">{h}</span>)}
              </div>
            </div>
          )}
              ...
              <div key={day.day} className="card-divine p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)]">{day.date} · {day.auspiciousTiming}</p>
                  </div>
                </div>
                ...
                {day.mandirs?.map(m => (
                  <div key={m.name} className="flex gap-3 mb-3 pb-3 border-b border-[var(--warm-sand)] last:border-0">
                    ...
                    <div>
                      <p className="font-medium text-[var(--indigo-deep)] text-sm inline-flex items-center gap-1"><Icon name="temple_hindu" size={14} /> {m.name}</p>
                      <p className="text-xs text-[var(--text-secondary)] mt-0.5">{m.activity} · {m.duration}</p>
                      {m.tips && <p className="text-xs text-[var(--saffron)] mt-0.5 inline-flex items-center gap-0.5"><Icon name="lightbulb" size={13} /> {m.tips}</p>}
                    </div>
                  </div>
                ))}

                <div className="grid grid-cols-3 gap-2 mt-3 text-xs text-[var(--text-secondary)]">
                  {day.travel && <div className="bg-[var(--warm-sand)] rounded-lg p-2"><p className="font-medium text-[var(--indigo-deep)] mb-0.5 inline-flex items-center gap-0.5"><Icon name="train" size={13} /> Travel</p><p>{day.travel.from} → {day.travel.to}</p><p>{day.travel.mode} · {day.travel.duration}</p></div>}
                  <div className="bg-[var(--warm-sand)] rounded-lg p-2"><p className="font-medium text-[var(--indigo-deep)] mb-0.5 inline-flex items-center gap-0.5"><Icon name="hotel" size={13} /> Stay</p><p>{day.accommodation}</p></div>
                  <div className="bg-[var(--warm-sand)] rounded-lg p-2"><p className="font-medium text-[var(--indigo-deep)] mb-0.5 inline-flex items-center gap-0.5"><Icon name="restaurant" size={13} /> Food</p><p>{day.meals}</p></div>
                </div>
              </div>
            ))}
          </div>
          ...
          {itinerary.estimatedCost && (
            <div className="card-divine p-5">
              <h3 className="font-bold text-[var(--indigo-deep)] mb-3 inline-flex items-center gap-1"><Icon name="payments" size={18} /> Estimated Cost (per person)</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {itinerary.packingList?.length > 0 && (
              <div className="card-divine p-5">
                <h3 className="font-bold text-[var(--indigo-deep)] mb-3 inline-flex items-center gap-1"><Icon name="backpack" size={18} /> Packing List</h3>
                <ul className="space-y-1">{itinerary.packingList.map(i => <li key={i} className="text-sm text-[var(--text-secondary)] flex gap-2"><span>•</span>{i}</li>)}</ul>
              </div>
            )}
            {itinerary.importantNotes?.length > 0 && (
              <div className="card-divine p-5">
                <h3 className="font-bold text-[var(--indigo-deep)] mb-3 inline-flex items-center gap-1"><Icon name="push_pin" size={18} /> Important Notes</h3>
                <ul className="space-y-1">{itinerary.importantNotes.map(n => <li key={n} className="text-sm text-[var(--text-secondary)] flex gap-2"><Icon name="warning" size={14} className="flex-shrink-0 text-amber-600" />{n}</li>)}</ul>
              </div>
            )}
          </div>
        </div>
