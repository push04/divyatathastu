  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // Products now arrive server-rendered, so the only thing that must
        // happen on the client is resolving the session. Re-fetching them would
        // repeat a query whose answer is already painted on the page.
        const needProducts = initialProducts.length === 0
