      ...
      <div style={{ background: 'linear-gradient(160deg, #fff7ed 0%, #ffedd5 50%, #fed7aa 100%)', borderBottom: '2px solid rgba(234,88,12,0.25)' }}>
        <div className="max-w-5xl mx-auto px-6 py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            <div>
              <p className="text-xs text-[#9a3412] tracking-[0.4em] uppercase mb-4" style={{ fontFamily: "var(--font-label)" }}>Sacred Puja Service</p>
              <h1 className="text-5xl font-black text-[#431407] mb-3 leading-tight" style={{ fontFamily: "var(--font-display)" }}>
                Mahaganpati
              </h1>
              <p className="text-lg text-[#9a3412] mb-5" style={{ fontFamily: "var(--font-display)" }}>
              ...
              <div className="flex gap-3 flex-wrap">
                <a href="#pujas" className="btn-divine px-7 py-3 inline-flex items-center gap-2">
                  <Icon name="temple_hindu" size={18} />
                  Book a Puja
                </a>
            ...
            <div className="flex justify-center">
              <div className="relative">
                <div className="w-56 h-56 rounded-full flex items-center justify-center shadow-2xl"
                  ...
                  <span className="text-8xl font-black text-white" style={{ fontFamily: "var(--font-display)" }}>ॐ</span>
                </div>
                <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 text-center">
                  <p className="text-lg font-black text-[#9a3412]" style={{ fontFamily: "var(--font-display)" }}>ॐ गं गणपतये नमः</p>
                </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 space-y-16">
        ...
        <section>
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-[var(--indigo-deep)]" style={{ fontFamily: "var(--font-display)" }}>The Forms of Mahaganpati</h2>
            <p className="text-sm text-[var(--text-secondary)] mt-2">Each attribute invoked for specific blessings</p>
          </div>
            ...
            {GANESH_ATTRIBUTES.map(a => (
              <div key={a.english} className="bento-card p-5 flex gap-4 items-start">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'linear-gradient(135deg, #fff7ed, #ffedd5)', border: '1px solid rgba(234,88,12,0.2)' }}>
                  <span className="text-[#9a3412] font-black text-lg" style={{ fontFamily: "var(--font-display)" }}>ॐ</span>
                </div>
                <div>
                  <div className="flex items-baseline gap-2 mb-1">
                    <h3 className="font-bold text-[#cc2200]" style={{ fontFamily: "var(--font-display)" }}>{a.sanskrit}</h3>
                    <span className="text-xs text-[var(--text-muted)]">({a.english})</span>
                  </div>
                  <p className="text-sm text-[var(--text-secondary)] leading-relaxed">{a.meaning}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section id="pujas">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-2xl font-bold text-[var(--indigo-deep)]" style={{ fontFamily: "var(--font-display)" }}>Sacred Puja Offerings</h2>
              <p className="text-sm text-[var(--text-secondary)] mt-1">All pujas performed with proper Vedic procedures and personalized sankalpa</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="bento-card p-10 text-center">
              <p className="text-[var(--text-muted)]">Puja schedules coming soon. Contact us to book directly.</p>
            </div>
          ) : (
                ...
                <div key={item.id} className="bento-card p-6 flex flex-col hover:shadow-lg transition-all hover:-translate-y-0.5">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex gap-1.5 flex-wrap">
                      {item.is_live && <span className="text-[12px] px-2 py-0.5 rounded-full bg-orange-100 text-orange-700 font-semibold">Live</span>}
                    ...
                    {item.duration && <span className="text-[13px] text-[var(--text-muted)] shrink-0" style={{ fontFamily: "var(--font-label)" }}>{item.duration}</span>}
                  </div>
                  ...
                  {!booked.has(item.id) && item.is_bookable && (
                    <div className="mb-4">
                      <label className="text-xs text-[var(--text-muted)] mb-1 block">Preferred date (optional)</label>
                    ...
                    {booked.has(item.id) ? (
                      ...
                      <button onClick={() => bookService(item)} disabled={booking === item.id || !item.is_bookable}
                        ...
                        {booking === item.id ? 'Booking...' : item.is_bookable ? (
                          <><Icon name="temple_hindu" size={16} />Book Puja</>
                        ) : 'Notify Me'}
                      </button>
                    )}
                  </div>
        ...
        <section>
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-[var(--indigo-deep)]" style={{ fontFamily: "var(--font-display)" }}>Blessings of Mahaganpati</h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {PUJA_BENEFITS.map(b => (
              <div key={b.title} className="bento-card p-5 text-center hover:shadow-md transition-all">
                <div className="mb-3"><Icon name={b.icon} size={30} className="text-[#9a3412]" /></div>
                <h3 className="font-bold text-[var(--indigo-deep)] mb-2 text-sm" style={{ fontFamily: "var(--font-display)" }}>{b.title}</h3>
        ...
        <section className="text-center py-10 rounded-2xl" style={{ background: 'linear-gradient(135deg, #fff7ed, #ffedd5)', border: '2px solid rgba(234,88,12,0.2)' }}>
          <div className="text-5xl mb-3 font-black text-[#9a3412]" style={{ fontFamily: "var(--font-display)" }}>ॐ</div>
          <h2 className="text-2xl font-bold text-[#431407] mb-2" style={{ fontFamily: "var(--font-display)" }}>Chaturthi - The Auspicious Day</h2>
          ...
          <div className="flex gap-3 justify-center flex-wrap">
            <a href="tel:9858784784" className="btn-divine px-7 py-3 inline-flex items-center gap-2">
              <Icon name="call" size={18} />Book Home Puja: 9858784784
            </a>
