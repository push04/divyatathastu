...
const BENEFITS = [
  ...
  { icon: 'brightness_7', title: 'Mantra Amplification', desc: 'Amplifies the potency of your daily japa and sadhana practice when used before practice' },
]

const FALLBACK_PRICE = 499
const WHATSAPP_NUMBER = '919858784784'

export default function NakshatraJalPage() {
    ...
    try {
      if (!product) {
        ...
        const msg = encodeURIComponent(
          `Hi! I'd like to order ${qty} bottle(s) of Nakshatra Jal. Please guide me on the next steps.`
        )
      ...
      await pay(
        ...
        {
          quantity: qty,
          notes: `Qty: ${qty} bottle(s) of Nakshatra Jal`,
  ...
  function handleWhatsAppOrder() {
    const msg = encodeURIComponent(
      `Hi! I'd like to order ${qty} bottle(s) of Nakshatra Jal (₹${totalPrice.toLocaleString('en-IN')}). Please guide me.`
    )
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${msg}`, '_blank')
  }
    ...
    <div className="min-h-screen bg-[var(--kutch-white)]">
      ...
      <div style={{ background: 'linear-gradient(160deg, #ecfdf5 0%, #d1fae5 40%, #a7f3d0 100%)', borderBottom: '2px solid rgba(16,185,129,0.3)' }}>
        <div className="max-w-5xl mx-auto px-5 sm:px-6 py-12 sm:py-14">
          <Link
            href="/ardra-jalam"
            className="inline-flex items-center gap-1.5 mb-6 text-sm font-semibold text-emerald-800/70 hover:text-emerald-900 transition-colors"
          >
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <div className="inline-flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-full bg-emerald-100 text-emerald-800 uppercase tracking-widest">
                  <Icon name="water_drop" size={14} /> Sacred Product · Limited Batches
                </div>
                <Link
                  href="/ebooks"
                ...
                >
                  <Icon name="auto_stories" size={14} className="shimmer-text" />
                  <span className="shimmer-text" style={{ fontFamily: "var(--font-display)" }}>Shop Sacred Ebooks</span>
                </Link>
              </div>
              <h1 className="text-4xl sm:text-5xl font-black mb-3 leading-tight" style={{ fontFamily: "var(--font-display)", color: '#065f46' }}>
                Nakshatra Jal
              </h1>
              <p className="text-xl text-[#065f46]/75 mb-2" style={{ fontFamily: "var(--font-display)" }}>
                Sacred Healing Water, charged under Ardra Nakshatra
              </p>
              <p className="text-base sm:text-lg text-[#065f46]/75 mb-6 leading-relaxed">
                ...
                {loading ? (
                  ...
                  <div className="inline-flex flex-col gap-1">
                    <div className="flex items-baseline gap-3">
                      <span className="text-5xl font-black text-[var(--terracotta)]" style={{ fontFamily: "var(--font-display)" }}>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-base text-[#065f46]/70 font-medium">per bottle (500ml)</span>
                      {product?.original_price && (
                        <span className="inline-flex items-center gap-1 bg-emerald-500 text-white text-[12px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1 bg-white rounded-2xl border-2 border-emerald-300 p-1 shadow-sm">
                      ...
                      >−</button>
                      <span className="w-10 text-center font-black text-[var(--indigo-deep)] text-lg" style={{ fontFamily: "var(--font-mono)" }}>{qty}</span>
                      <button
              ...
              <div className="relative">
                ...
                <div className="w-48 h-64 bg-gradient-to-b from-[#a7f3d0] via-[#6ee7b7] to-[#34d399] rounded-[40px] flex items-center justify-center shadow-2xl relative overflow-hidden">
                  ...
                  <div className="text-center text-white relative z-10">
                    <Icon name="water_drop" size={56} className="text-white block mb-2" />
                    <div className="text-sm font-bold tracking-widest">NAKSHATRA</div>
                    <div className="text-xs opacity-80 tracking-widest">JAL</div>
                    <div className="text-[12px] opacity-60 mt-2">500ml</div>
                  </div>
                  {/* Water ripple effect */}
                  <div className="absolute bottom-0 left-0 right-0 h-16 bg-white/10 rounded-t-full" />
                </div>
                {/* Floating badge */}
                <div className="absolute -top-3 -right-3 bg-[var(--terracotta)] text-white text-[12px] font-bold px-3 py-1.5 rounded-full shadow-lg uppercase tracking-widest">
                  Limited
                </div>
                <div className="absolute -bottom-4 left-1/2 -translate-x-1/2"><Icon name="water_drop" size={31} className="text-emerald-300" /></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-5 sm:px-6 py-12 space-y-14 sm:space-y-16">
        ...
        <section>
          <div className="text-center mb-8">
            <h2 className="text-3xl font-black text-[var(--indigo-deep)]" style={{ fontFamily: "var(--font-display)" }}>The Science of Ardra Nakshatra</h2>
            <p className="text-lg text-[var(--text-secondary)] mt-2">Why this nakshatra makes the water sacred</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {NAKSHATRA_INFO.map(n => (
              <div key={n.title} className="bento-card p-5 flex gap-4">
                <div className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center shrink-0"><Icon name="water_drop" size={20} className="text-emerald-600" /></div>
                <div>
                  <h3 className="font-bold text-[var(--indigo-deep)] mb-1" style={{ fontFamily: "var(--font-display)" }}>{n.title}</h3>
                  <p className="text-base text-[var(--text-secondary)] leading-relaxed">{n.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Benefits */}
        <section style={{ background: 'linear-gradient(135deg, #f0fdf4, #ecfdf5)', borderRadius: 20 }} className="p-6 sm:p-10">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-black text-[var(--indigo-deep)]" style={{ fontFamily: "var(--font-display)" }}>Benefits</h2>
            <p className="text-lg text-[var(--text-secondary)] mt-2">As per Vedic tradition and reported experiences</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {BENEFITS.map(b => (
              <div key={b.title} className="bg-white rounded-2xl p-5 shadow-sm">
                <div className="mb-3"><Icon name={b.icon} size={24} className="text-emerald-700" /></div>
                <h3 className="font-bold text-[var(--indigo-deep)] mb-2 text-lg">{b.title}</h3>
        ...
        <section>
          <div className="text-center mb-8">
            <h2 className="text-3xl font-black text-[var(--indigo-deep)]" style={{ fontFamily: "var(--font-display)" }}>How to Use</h2>
          </div>
          <div className="space-y-4">
            {HOW_TO_USE.map(h => (
              <div key={h.step} className="bento-card p-5 flex items-start gap-5">
                <div className="w-12 h-12 rounded-2xl bg-[var(--indigo-deep)] flex items-center justify-center text-white font-black text-lg shrink-0 shadow-md" style={{ fontFamily: "var(--font-mono)" }}>
                  {h.step}
                </div>
                <div>
                  <h3 className="text-lg font-extrabold text-[var(--indigo-deep)] mb-1">{h.title}</h3>
                  <p className="text-base text-[var(--text-secondary)] leading-relaxed">{h.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {product?.long_description && (
          <section className="bento-card p-8">
            <h2 className="text-xl font-bold text-[var(--indigo-deep)] mb-5" style={{ fontFamily: "var(--font-display)" }}>
          ...
          <div className="relative z-10">
            <div className="mb-4 flex justify-center">
              <div className="w-16 h-16 rounded-full bg-white/10 flex items-center justify-center">
                <Icon name="water_drop" size={46} className="text-emerald-300" />
              </div>
            </div>
            <h2 className="text-3xl font-black text-white mb-2" style={{ fontFamily: "var(--font-display)" }}>Ready to Begin?</h2>
            <p className="text-lg text-[var(--text-on-dark)] mb-8 max-w-lg mx-auto leading-relaxed">
            ...
            {!booked ? (
              <div className="flex flex-col items-center gap-4">
                  ...
                  >
                    {ordering ? (
                      ...
                      <>
                        ...
                        <span>Buy Nakshatra Jal - ₹{unitPrice.toLocaleString('en-IN')}</span>
                ...
                <div className="flex flex-wrap justify-center items-center gap-x-6 gap-y-2 text-[var(--text-on-dark-secondary)] text-sm">
                  <span className="flex items-center gap-1"><Icon name="verified" size={14} /> Secure Payment</span>
                  <span className="flex items-center gap-1"><Icon name="local_shipping" size={14} /> Pan India Delivery</span>
                  <span className="flex items-center gap-1"><Icon name="stars" size={14} /> Vedic Certified</span>
                </div>
            ...
            <p className="text-xs text-[var(--text-on-dark-muted)] mt-6">📞 9858784784 · info@mahatathastu.com</p>
          </div>
        </section>
      </div>
      {NoticeModal}
