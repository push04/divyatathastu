    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="flex justify-center mb-4"><Icon name="temple_hindu" size={64} className="text-[var(--indigo-deep)]" /></div>
      ...
      <div className="relative h-56 overflow-hidden" style={{ background: 'linear-gradient(135deg, var(--indigo-deep) 0%, var(--plum) 55%, #1a0a2e 100%)' }}>
        ...
        <div className="relative z-10 h-full flex items-center justify-center">
          <div className="icon-divine w-24 h-24 rounded-3xl shadow-2xl shadow-[var(--terracotta)]/30">
            <Icon name={categoryIcon} size={52} className="text-white" />
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div>
            <div className="flex gap-2 mb-3 flex-wrap">
              ...
              <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${CAT_COLOR[event.category] || 'bg-[var(--warm-sand)] text-[var(--text-secondary)]'}`}>{event.category}</span>
            </div>
          ...
          <div className="card-divine p-5 grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-semibold text-sm flex items-center gap-1"><Icon name="calendar_today" size={14} />{(() => { const [y,m,d] = event.start_date.split('-'); return new Date(+y,+m-1,+d).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) })()}</p>
              <p className="text-sm text-[var(--text-secondary)] flex items-center gap-1"><Icon name="schedule" size={14} />{event.start_time} IST</p>
            </div>
            <div>
              <p className="text-xs text-[var(--text-muted)] mb-1">Duration</p>
              <p className="font-semibold text-sm flex items-center gap-1"><Icon name="timer" size={14} />{event.duration_minutes >= 60 ? `${Math.floor(event.duration_minutes / 60)}h ${event.duration_minutes % 60 ? event.duration_minutes % 60 + 'm' : ''}` : `${event.duration_minutes}m`}</p>
            </div>
            <div>
              <p className="text-xs text-[var(--text-muted)] mb-1">Location</p>
              <p className="font-semibold text-sm flex items-center gap-1"><Icon name="location_on" size={14} /> {event.location}</p>
            </div>
            <div>
              <p className="text-xs text-[var(--text-muted)] mb-1">Hosted by</p>
              <p className="font-semibold text-sm flex items-center gap-1"><Icon name="person" size={14} /> {event.host || 'MahaTathastu Team'}</p>
            </div>
          </div>
          ...
          {event.includes && (
            <div className="card-divine p-5">
              <h2 className="text-lg font-bold text-[var(--indigo-deep)] mb-3" style={{ fontFamily: "var(--font-display)" }}>What's Included</h2>
              <ul className="space-y-2">
                {event.includes.map((item: string, i: number) => (
                  <li key={i} className="flex items-center gap-2 text-sm text-[var(--text-primary)]">
                    <Icon name="check_circle" size={14} className="text-emerald-500" /> {item}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {event.requirements && (
            <div className="card-divine p-5">
              <h2 className="text-lg font-bold text-[var(--indigo-deep)] mb-2" style={{ fontFamily: "var(--font-display)" }}>Requirements</h2>
              <p className="text-sm text-[var(--text-secondary)]">{event.requirements}</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          <div className="card-divine p-5 sticky top-24">
            <div className="text-center mb-4">
              ...
              {event.price > 0 && <p className="text-xs text-[var(--text-muted)]">per person</p>}
            </div>

            <div className="mb-4">
              <div className="flex justify-between text-xs mb-1">
                <span className="text-[var(--text-muted)]">{event.current_participants}/{event.max_participants} registered</span>
            ...
            <p className="text-xs text-center text-[var(--text-muted)] mt-3">Secure payment · Instant confirmation</p>
          </div>

          <Link href="/events" className="flex items-center justify-center gap-1 text-sm text-[var(--indigo-deep)] hover:underline"><Icon name="arrow_back" size={16} />Back to all events</Link>
        </div>
      </div>
    </div>
  )
